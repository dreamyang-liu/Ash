...
//! Integrates with clip/paste for easy content transfer.
...
pub struct BufferToClipArgs {
    ...
    #[serde(default)]
    pub start_line: Option<usize>,
    /// End line (inclusive, default: end)
    #[serde(default)]
    pub end_line: Option<usize>,
    /// Clip name to save as
    pub clip_name: String,
}

pub struct BufferToClipTool;

impl Tool for BufferToClipTool {
    fn name(&self) -> &'static str { "buffer_to_clip" }
    fn description(&self) -> &'static str { "Copy buffer range to clipboard" }
    ...
    fn schema(&self) -> Value {
        serde_json::json!({
            ...
            "required": ["clip_name"],
            "properties": {
                "buffer": {"type": "string", "description": "Buffer name (default: 'main')"},
                "start_line": {"type": "integer", "description": "Start line (default: 1)"},
                "end_line": {"type": "integer", "description": "End line (default: end)"},
                "clip_name": {"type": "string", "description": "Name for the clip"}
            }
        })
    }
    ...
    fn execute(&self, args: Value) -> BoxFuture<'_, ToolResult> {
        Box::pin(async move {
            let args: BufferToClipArgs = match serde_json::from_value(args) {
                Ok(a) => a,
                Err(e) => return ToolResult::err(format!("Invalid args: {e}")),
            };

            let buffer_name = args.buffer.as_deref().unwrap_or(DEFAULT_BUFFER);
            let store = load_store().await;

            let buffer = match store.buffers.get(buffer_name) {
                Some(b) => b,
                None => return ToolResult::err(format!("Buffer '{}' not found", buffer_name)),
            };
            ...
            if start > buffer.lines.len() {
                return ToolResult::err(format!("Start line {} exceeds buffer length", start));
            }

            let content: String = buffer.lines[(start - 1)..end].join("\n");
            let source = format!("buffer:{}:{}-{}", buffer_name, start, end);

            // Save to clip using the clip module's storage
            use super::clip::{save_clip_entry};
            match save_clip_entry(&args.clip_name, &content, Some(&source)).await {
                Ok(_) => ToolResult::ok(format!(
                    "Copied {} lines from buffer '{}' to clip '{}'",
                    end - start + 1, buffer_name, args.clip_name
                )),
...
#[derive(Debug, Clone, Deserialize, Default)]
pub struct ClipToBufferArgs {
    /// Clip name to read from
    pub clip_name: String,
    /// Buffer name (default: "main")
    #[serde(default)]
    pub buffer: Option<String>,
    /// Line to insert at (1-indexed). If omitted, replaces buffer content.
    #[serde(default)]
    pub at_line: Option<usize>,
    /// Append to end instead of inserting
    #[serde(default)]
    pub append: bool,
}

pub struct ClipToBufferTool;

impl Tool for ClipToBufferTool {
    fn name(&self) -> &'static str { "clip_to_buffer" }
    fn description(&self) -> &'static str { "Paste clipboard content into buffer" }
    ...
    fn schema(&self) -> Value {
        serde_json::json!({
            ...
            "required": ["clip_name"],
            "properties": {
                "clip_name": {"type": "string", "description": "Clip name to paste from"},
                "buffer": {"type": "string", "description": "Buffer name (default: 'main')"},
                "at_line": {"type": "integer", "description": "Insert at line (omit to replace)"},
                "append": {"type": "boolean", "description": "Append to end"}
            }
        })
    }
    ...
    fn execute(&self, args: Value) -> BoxFuture<'_, ToolResult> {
        Box::pin(async move {
            let args: ClipToBufferArgs = match serde_json::from_value(args) {
                Ok(a) => a,
                Err(e) => return ToolResult::err(format!("Invalid args: {e}")),
            };

            // Load clip content
            use super::clip::load_clip_entry;
            let content = match load_clip_entry(&args.clip_name).await {
                Ok(c) => c,
                Err(e) => return ToolResult::err(e),
            };

            let buffer_name = args.buffer.as_deref().unwrap_or(DEFAULT_BUFFER).to_string();
            let new_lines: Vec<String> = content.lines().map(String::from).collect();
            let new_count = new_lines.len();

            let mut store = load_store().await;
            let buffer = store.buffers.entry(buffer_name.clone()).or_insert_with(Buffer::new);
            ...
            ToolResult::ok(format!(
                "Pasted clip '{}' ({} lines) into buffer '{}' ({})",
                args.clip_name, new_count, buffer_name, action
            ))
        })
    }
}
